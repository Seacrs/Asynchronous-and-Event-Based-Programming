...
```
## 12. Create a function called fetchUserTodos that uses the Promise.all() method to fetch users and todos concurrently from the provided API endpoints and combine them based on the userId. The function should return a promise that resolves with the combined data.
```
...
### solution
```bash
async function promisifyfetch(user, todo){
  try{ 
    const p = await Promise.all([fetch(user).then(response => response.json()), fetch(todo).then(response => response.json())])
    .then(([users, todos])=>{
...
promisifyfetch('https://jsonplaceholder.typicode.com/users','https://jsonplaceholder.typicode.com/todos');
```